...
const sDOM = (tagName, attrs = {}, children, existingRoot) => {
  const elem =
    existingRoot || doc.createElementNS("http://www.w3.org/2000/svg", tagName);
...
export default class Pattern {
  ...
  _toSVG = (serializer, destSVG, _svgOpts = {}) => {
    ...
    const roundedPoints =
      svgOpts.coordinateDecimals < 0
        ? points
        : points.map((p) =>
            p.map((x) => +x.toFixed(svgOpts.coordinateDecimals))
          );

    const paths = polys.map((poly) => {
      const xys = poly.vertexIndices.map(
        (i) => `${roundedPoints[i][0]},${roundedPoints[i][1]}`
      );
    ...
    const svg = s(
      "svg",
      {
        xmlns: svgOpts.includeNamespace
