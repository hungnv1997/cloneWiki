  ...
  <div>
    <div class="left-handler">
      <el-tooltip placement="top" :hide-after="0">
        <template #content>{{ MULTILANG.header.redo }}</template>
        <IconBack
          class="handler-item"
      ...
      <el-tooltip placement="top" :hide-after="0">
        <template #content>{{ MULTILANG.header.undo }}</template>
        <IconNext
          class="handler-item"
      ...
      <el-tooltip placement="top" :hide-after="0">
        <template #content>{{ MULTILANG.header.combination }}</template>
        <IconGroup
          class="handler-item"
      ...
      <el-tooltip placement="top" :hide-after="0">
        <template #content>{{ MULTILANG.header.ungroup }}</template>
        <IconUngroup
          class="handler-item"
    ...
    <div class="center-handler">
      <el-tooltip placement="top" :content="MULTILANG.header.layer">
        <IconLayers class="handler-item" ref="layerRef" />
      </el-tooltip>
      <el-popover
        ref="layerPopoverRef"
        :virtual-ref="layerRef"
        trigger="click"
        virtual-triggering
        width="240"
      >
        <el-row class="handler-icon-row">
          <el-button-group>
            ...
            <el-tooltip
              placement="top"
              :hide-after="0"
              :content="MULTILANG.header.bottom"
            >
            ...
            <el-tooltip
              placement="top"
              :hide-after="0"
              :content="MULTILANG.header.move_down"
            >
            ...
            <el-tooltip
              placement="top"
              :hide-after="0"
              :content="MULTILANG.header.move_up"
            >
      ...
      <el-tooltip placement="top" :content="MULTILANG.header.align">
        <IconAlignTextCenter class="handler-item" ref="alignRef" />
      </el-tooltip>
      <el-popover
        ref="alignPopoverRef"
        :virtual-ref="alignRef"
        trigger="click"
        :width="200"
        virtual-triggering
      >
        <el-row class="handler-icon-row">
          <el-button-group>
            <el-tooltip
              placement="top"
              :hide-after="0"
              :content="MULTILANG.header.align_left"
            >
            ...
            <el-tooltip
              placement="top"
              :hide-after="0"
              :content="MULTILANG.header.align_center"
            >
              <el-button @click="verticallyAlign"
                ><IconAlignVertically
              /></el-button>
            </el-tooltip>
            <el-tooltip
              placement="top"
              :hide-after="0"
              :content="MULTILANG.header.align_right"
            >
        ...
        <el-row class="handler-icon-row mt-10">
          <el-button-group>
            <el-tooltip
              placement="top"
              :hide-after="0"
              :content="MULTILANG.header.align_top"
            >
            ...
            <el-tooltip
              placement="top"
              :hide-after="0"
              :content="MULTILANG.header.align_middle"
            >
              <el-button @click="horizontallyAlign"
            ...
            <el-tooltip
              placement="top"
              :hide-after="0"
              :content="MULTILANG.header.align_bottom"
            >
        ...
        <el-row>
          ...
          <el-col :span="7" class="position-input">
            <el-input-number
              ...
              v-model="rotate"
              :max="360"
              @change="changeRotate(rotate)"
            ></el-input-number>
          </el-col>
          <el-col :span="2" class="position-text"></el-col>
          <el-button-group>
            <el-button @click="changeRotate45('-')"
              ><IconRotate /> -45°</el-button
            >
            <el-button @click="changeRotate45('+')"
...
watch(
  ...
  () => {
    ...
    canUnGroup.value = false;
    if (!handleElement.value) return;
    rotate.value = handleElement.value.angle
      ? handleElement.value.angle
      : rotate.value;
    lock.value =
      handleElement.value.lockMovementX && handleElement.value.lockMovementX
        ? true
        : false;
...
const leftAlign = () => {
  ...
  if (!workSpaceDraw.left || !workSpaceDraw.width || !handleElement.value.width)
    return;
...
const rightAlign = () => {
  ...
  if (!workSpaceDraw.left || !workSpaceDraw.width || !handleElement.value.width)
    return;
  handleElement.value.left =
    workSpaceDraw.left + workSpaceDraw.width - handleElement.value.width / 2;
};
// 上对齐
const topAlign = () => {
  if (!handleElement.value) return;
  ...
  if (
    !workSpaceDraw.top ||
    !workSpaceDraw.height ||
    !handleElement.value.height
  )
    return;
...
const bottomAlign = () => {
  ...
  handleElement.value.top =
    workSpaceDraw.top + workSpaceDraw.height - handleElement.value.height / 2;
};
