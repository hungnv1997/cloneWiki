  ...
  >
    <div class="thumb-handle">
      <div class="btn" @click="createTemplate()">
        <IconPlus class="icon" />{{ MULTILANG.content_thumb.editor.add_page }}
      </div>
    </div>
    <Draggable
      class="thumb-content"
...
const {
  selectedTemplatesIndex: _selectedTemplatesIndex,
  thumbnailsFocus,
  canvasObject,
...
const selectedTemplatesIndex = computed(() => [
  ..._selectedTemplatesIndex.value,
  templateIndex.value,
]);
...
const handleClickTemplateThumbnail = (e: MouseEvent, index: number) => {
  ...
  if (
    isMultiSelected &&
    selectedTemplatesIndex.value.includes(index) &&
    e.button !== 0
  )
    return;
  ...
  if (ctrlKeyState.value) {
    if (templateIndex.value === index) {
      ...
      if (selectedTemplatesIndex.value.includes(index)) {
        const newSelectedSlidesIndex = selectedTemplatesIndex.value.filter(
          (item) => item !== index
        );
        // mainStore.updateSelectedSlidesIndex(newSelectedSlidesIndex)
      } else {
